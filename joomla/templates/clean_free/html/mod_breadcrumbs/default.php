	...
	<ul class="uk-breadcrumb">
        ...
        <?php else : ?>
			<li class="mod-breadcrumbs__divider">
				<span class="divider <?php echo JVERSION < 4 ? 'fas fa-map-marker-alt' : 'icon-location icon-fw'; ?>" aria-hidden="true"></span>
			</li>
        <?php endif; ?>
        ...
        foreach ($list as $key => $item) :
            if ($key !== $last_item_key) :
                ...
                echo '<li class="mod-breadcrumbs__item' . $class . '">' . $breadcrumbItem . '</li>';
            elseif ($show_last) :
